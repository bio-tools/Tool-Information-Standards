...
- [Tool Information Profile](http://github.com/bio-tools/tool-information-profile) lists software attributes (from biotoolsSchema) that should be specified for different ypes of tools within a set of tool descriptions. It provides a practical **framework** and **metrics** for description of individual tools, and the curation of collections such as [bio.tools](https://bio.tools).
